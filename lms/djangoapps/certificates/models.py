...
from util.milestones_helpers import collect_course_milestone
...
def handle_post_cert_generated(sender, instance, **kwargs):  # pylint: disable=no-self-argument, unused-argument
    """
    Handles pos_save signal of GeneratedCertificate, and mark user collected
    ...
    if settings.FEATURES.get('MILESTONES_APP') \
            and settings.FEATURES.get('ENABLE_PREREQUISITE_COURSES') \
            and instance.status == CertificateStatuses.generating:
        collect_course_milestone(instance.course_id, instance.user)
